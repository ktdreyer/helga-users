...
def read_module_contents():
    with open('helga_users/__init__.py') as init:
        return init.read()
...
class BumpCommand(Command):
    ...
    def run(self):
        ...
        module_file = read_module_contents()
        with open('helga_users/__init__.py', 'w') as fileh:
...
class ReleaseCommand(Command):
    ...
    def run(self):
        ...
        tarball = 'dist/%s-%s.tar.gz' % ('helga-users', version)
...
class PyTest(TestCommand):
    ...
    def run_tests(self):
        import pytest
        errno = pytest.main('helga_users/tests', self.pytest_args)
        sys.exit(errno)


setup(name="helga-users",
      version=version,
      description='Users plugin for Helga',
      ...
      keywords='irc bot users',
      author='ken dreyer',
      author_email='ktdreyer@ktdreyer.com',
      url='https://github.com/ktdreyer/helga-users',
      license='MIT',
      ...
      entry_points=dict(
          helga_plugins=[
              'users = helga_users.command:helga_users',
          ],
      ),
